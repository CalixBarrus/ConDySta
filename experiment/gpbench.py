...
from experiment.common import format_num_secs, setup_dirs_with_ic3
import hybrid
from hybrid.flowdroid import run_flowdroid, run_flowdroid_help, run_flowdroid_paper_settings
...
from util.input import BatchInputModel, input_apks_from_dir
from util.subprocess import run_command_direct

import pandas as pd

from util import logger
logger = logger.get_logger('experiment', 'gpbench')
...
def single_gpbench_experiment(**file_paths):
    experiment_args = gpbench_experiment_setup_misc(**filepaths):
...
def gpbench_experiment_setup_base(**file_paths) -> Dict[str, typing.any]:
    ...
    flowdroid_args = FlowdroidArgs(**FlowdroidArgs.default_settings) # TODO: add new gbpench settings
...
def gpbench_experiment_setup_misc(**filepaths) -> Dict[str, typing.Any]:
    ...
    experiment_args["experiment_name"] = "fd-on-gpbench-temp"
    experiment_args["experiment_description"] = "Run Flowdroid on Google Play Bench"
...
"""
    ...
    input_apks: BatchInputModel = apps_from_gpbench(gpbench_dir_path, results_df)
    assign_input_gpbench_benchmark_ids(input_apks, results_df)


    # Only use a subset of inputs
    mask = results_df["AppID"].isin(range(1,20))
    mask = results_df["AppID"].isin([3,6,8,9,10,15,19])
...
def flowdroid_on_gpbench_full(**kwargs):
    """
    use_model_paths_csv: whether to try and run ic3 for each app, or to use precomputed ones specified in a file hard coded below
    using_ic3_script: whether the ic3_path points to the folder with the script from Junbin Zhang, or to a jar file
    """
    ...
    timeout = experiment_args["timeout"] 
    ...
    flowdroid_args = experiment_args["flowdroid_args"] 
    always_new_experiment_directory = experiment_args["always_new_experiment_directory"] 


    # experiment_name = "fd-2.13-on-gpbench-no-ic3"
    # ic3_timeout = 4 * 60 * 60
    ic3_timeout = 15 * 60
    flowdroid_timeout = 15 * 60
    # flowdroid_timeout = 72 * 60 * 60  
    ...
    gpbench_description_path = "data/benchmark-descriptions/gpbench-info.csv"
    results_df = pd.read_csv(gpbench_description_path, header=0, index_col=False)
    results_df = results_df.set_index(results_df["AppID"])
    results_df["Detected Flows"] = None
    results_df["Error Message"] = None
    # results_df["IC3 Time"] = None
    results_df["Time Elapsed"] = None
    ...
    results_df["Input Model"] = None
    for input_model in input_apks.ungrouped_inputs:
        mask = results_df["AppName"].apply(lambda x: x in input_model.apk().apk_name)
        # only one result should be found 
        if mask.sum() > 1:
            raise AssertionError("Apk Name " + input_model.apk().apk_name + " found in multiple df rows: \n" + str(input_apks[mask]))
        app_id = input_apks[mask]["AppID"].iloc[0]
        input_model.benchmark_id = app_id
        results_df.loc[app_id, "Input Model"] = input_model
    ...
    # for input_model in input_apks.ungrouped_inputs:
        input_model = results_df.loc[app_id, "Input Model"]
        ...
        try:
            ...
            results_df.loc[input_model.benchmark_id, "Time Elapsed"] = time.time() - t0
        except TimeoutExpired as e:
            logger.error(f"Flowdroid timed out after {flowdroid_timeout} seconds; details in " + fd_log_path)
            results_df.loc[input_model.benchmark_id, "Error Message"] = f"Flowdroid Timed Out after {format_num_secs(flowdroid_timeout)} "
            continue


        leaks_count = get_reported_num_leaks_in_flowdroid_log(fd_log_path)
        if leaks_count is None:
            results_df.loc[input_model.benchmark_id, "Error Message"] = "Flowdroid Irregularity, review log at " + fd_log_path
        results_df.loc[input_model.benchmark_id, "Detected Flows"] = leaks_count
