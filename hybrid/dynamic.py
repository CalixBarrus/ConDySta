...
class AbstractDynamicLogProcessingStrategy:
    def sources_from_log(self, hybrid_config: HybridAnalysisConfig, log_path: str, apk_name: str):
...
def dynamic_log_processing_strategy_factory(hybrid_config: HybridAnalysisConfig) -> AbstractDynamicLogProcessingStrategy:
...
class StringReturnDynamicLogProcessingStrategy(AbstractDynamicLogProcessingStrategy):
    def sources_from_log(self, hybrid_config: HybridAnalysisConfig, log_path: str, apk_name: str) -> Set[
        'MethodSignature']:
    ...
    def source_sink_file_from_sources(self, hybrid_config: HybridAnalysisConfig,
                                      sources: Set['MethodSignature'], apk_name: str) -> str:
        unmodified_source_and_sink_path = hybrid_config.unmodified_source_sink_list_path
...
class InstrReportReturnOnlyDynamicLogProcessingStrategy(AbstractDynamicLogProcessingStrategy):
    ...
    def source_sink_file_from_sources(self, hybrid_config: HybridAnalysisConfig,
                                      sources: Set['MethodSignature'], apk_name: str):
    ...
    def _instr_reports_to_sources(self, instr_reports: List[InstrumentationReport]) -> Set[
        'MethodSignature']:
        ...
        for instr_report in instr_reports:
            ...
            if instr_report.is_arg and (instr_report.arg_index >= len(pre_invoke_arg_masks[instr_report.invoke_id])):
                additional_mask_length = (instr_report.arg_index+1) - len(pre_invoke_arg_masks[instr_report.invoke_id])
                pre_invoke_arg_masks[instr_report.invoke_id] = ([False] * additional_mask_length) + pre_invoke_arg_masks[instr_report.invoke_id]
                post_invoke_arg_masks[instr_report.invoke_id] = ([False] * additional_mask_length) + post_invoke_arg_masks[instr_report.invoke_id]

            if not instr_report.is_before_invoke:
                # return value
                if not instr_report.is_arg:
...
class InstrReportReturnAndArgsDynamicLogProcessingStrategy(AbstractDynamicLogProcessingStrategy):
    ...
    def sources_from_log(self, hybrid_config: HybridAnalysisConfig, log_path: str,
                         apk_name: str) -> SourceSinkXML:

        log = LogcatLogFileModel(log_path)

        instr_reports = log.scan_log_for_instrumentation_reports()

        source_sink_xml = self._instr_reports_to_source_sink(instr_reports)
        discovered_sources_count = source_sink_xml.source_count
    ...
    def source_sink_file_from_sources(self, hybrid_config: HybridAnalysisConfig,
                                      sourceSinks: SourceSinkXML, apk_name: str):
    ...
    def _instr_reports_to_source_sink(self, instr_reports: List[InstrumentationReport]) -> SourceSinkXML:
        ...
        for instr_report in instr_reports:
            ...
            if not instr_report.is_before_invoke:
                ...
                if not instr_report.is_arg:
                    ...
                    post_invoke_arg_masks[instr_report.invoke_id][instr_report.arg_index] = True
            else:
                if not instr_report.is_arg:
                    raise AssertionError("Case should not occur, can't have a return "
                                         "value before the invocation")
                else:
                    pre_invoke_arg_masks[instr_report.invoke_id][instr_report.arg_index] = True

        return_source_signatures: List[str] = []
        arg_source_signatures: List[str] = []
        ...
        for return_source_signature in return_source_signatures:
            result_source_sinks.add_return_source(
                MethodSignature.from_signature_string(return_source_signature))

        for arg_source_signature, arg_source_index in zip(arg_source_signatures, arg_source_indices):
            result_source_sinks.add_arg_source(MethodSignature.from_signature_string(arg_source_signature), arg_source_index)
    ...
    def scan_log_for_instrumentation_reports(self) -> List['InstrumentationReport']:
        ...
        for line in self.lines:
            if report_preamble in line:
                ...
                instr_report_string, access_path, private_string = re_result.group(1), re_result.group(2), re_result.group(3)
